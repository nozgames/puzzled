...
namespace Puzzled
{
    public class DatabaseManager : MonoBehaviour
    {
        ...
        private Sprite GeneratePreview(Tile prefab)
        {
            ...
            _tilePreviewCamera.transform.localEulerAngles = new Vector3(tile.info.previewPitch,0,0);
            _tilePreviewCamera.orthographic = tile.info.previewOrthographic;
            _tilePreviewCamera.orthographicSize = size * 0.5f * tile.info.previewSize;
            _tilePreviewCamera.transform.position = CameraManager.Frame(
                tile.transform.position + new Vector3(0, (max.y + min.y) * 0.5f, 0),
                tile.info.previewPitch,
                0,
                size * tile.info.previewSize,
