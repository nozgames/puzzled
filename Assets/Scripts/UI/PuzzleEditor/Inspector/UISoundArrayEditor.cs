...
{
    class UISoundArrayEditor : UIPropertyEditor, IInspectorStateProvider
    {
        [SerializeField] private GameObject _itemPrefab = null;
        [SerializeField] private UIList _items = null;

        private List<Sound> _values;
        ...
        protected override void OnTargetChanged()
        {
            ...
            _values = target.GetValue<Sound[]>()?.ToList() ?? new List<Sound>();
        ...
        private void RemoveItem(UISoundArrayEditorItem item)
        {
        ...
        private UISoundArrayEditorItem AddValue(Sound value)
        {
            var editor = Instantiate(_itemPrefab, _items.transform).GetComponent<UISoundArrayEditorItem>();
            editor.onValueChanged += (v) =>
            {
                var option = ((TilePropertyEditorTarget)target);
                _values[editor.transform.GetSiblingIndex()] = v.value;
